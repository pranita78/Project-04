...
public class ShiftListCtl extends BaseCtl {
	...
	Logger log = Logger.getLogger(ShiftListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<ShiftBean> list = model.search(bean, pageNo, pageSize);
			List<ShiftBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op)
				...
				if (ids != null && ids.length > 0) {
					ShiftBean deleteBean = new ShiftBean();
					for (String id : ids) {
