...
import in.co.rays.proj4.model.ShiftModel;
...
public class ShiftCtl extends BaseCtl {
    ...
    Logger log = Logger.getLogger(ShiftCtl.class);
    ...
    protected boolean validate(HttpServletRequest request) {
        ...
        if (DataValidator.isNull(request.getParameter("shiftCode"))) {
            request.setAttribute("shiftCode",
                    PropertyReader.getValue("error.require", "Shift Code"));
            pass = false;
        }

        if (DataValidator.isNull(request.getParameter("shiftName"))) {
            request.setAttribute("shiftName",
                    PropertyReader.getValue("error.require", "Shift Name"));
            pass = false;
        }
    ...
    protected BaseBean populateBean(HttpServletRequest request) {
        ...
        bean.setShiftCode(DataUtility.getString(request.getParameter("shiftCode")));
        bean.setShiftName(DataUtility.getString(request.getParameter("shiftName")));
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (id > 0) {
            try {
                ShiftBean bean = model.findByPk(id);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (OP_SAVE.equalsIgnoreCase(op)) {
            ...
            ShiftBean bean = (ShiftBean) populateBean(request);
